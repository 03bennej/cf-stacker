    ...
    def fit(self, X, y):
        ...
        if self.nmf:
            self.nmf_train = NMF(n_components=self.latent_dimension,
                                 ...
                                 update_H=True)
            np.random.seed(73863); W_init = 0.1 * np.random.rand(X.shape[0], self.latent_dimension)
            np.random.seed(73863); H_init = 0.1 * np.random.rand(self.latent_dimension, X.shape[1])
