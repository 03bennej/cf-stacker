...
def define_variables(X_shape, latent_dim):
    ...
    omega = tf.Variable(tf.zeros([latent_dim, 1]),
                        dtype=tf.dtypes.float32,
                        trainable=True)
...
class MatrixFactorizationClassifier(BaseEstimator):
    ...
    def predict(self, X):
        ...
        self.y_predict = logistic_regression(self.W_test, self.omega, self.beta).numpy()[:, 0]

        return self.y_predict

    def train_losses(self, X, Xh, y, yh, W, H, omega):
        loss_mf = wmse(X, Xh, self.C_train)
        mf_reg = l2_reg(W, self.lam_WH) + l2_reg(H, self.lam_WH) + l2_reg(self.bw_train, self.lam_WH) + l2_reg(self.bh_train, self.lam_WH)
        loss_lr = bce_loss(y, yh) + l2_reg(omega, self.lam_omega)
        combined_loss = self.alpha * loss_mf + mf_reg + loss_lr
        return combined_loss, self.alpha * loss_mf + mf_reg, loss_lr
    ...
    def optimization_train_step(self, X_train, y):
        ...
        with tf.GradientTape() as tape:
            ...
            self.yh_train = logistic_regression(self.W_train, self.omega, self.beta)
    ...
    def optimization_test_step(self, X_train, X_test):
        with tf.GradientTape() as tape:
            ...
            self.yh_test = logistic_regression(self.W_test, self.omega, self.beta)
    ...
    def optimize_test(self, X_train, X_test):
        step = 0
        self.Xh_test = model(self.W_test, self.H, self.mu_train, self.bw_test, self.bh_train)
        self.yh_test = format_lr(logistic_regression(self.W_test, self.omega, self.beta))  # should be Xh
        self.C_test = calc_C(X_test, self.yh_test)
        mf_loss = self.test_loss(X_train, self.Xh_train, self.W_train, self.H, self.C_train) \
                  + self.test_loss(X_test, self.Xh_test, self.W_test, self.H, self.C_test)
...
if __name__ == "__main__":
    ...
    mf_model = MatrixFactorizationClassifier(latent_dim=30,
                                             alpha=1000,
                                             max_iter=200,
                                             learning_rate=0.01,
                                             tol=0.0000000001,
    ...
    # %%
    mf_model.max_iter = 200
    y_pred = mf_model.predict(X_test)
    
    Xh_test = mf_model.Xh_test

    sk_lr = LogisticRegression(max_iter=600)
    sk_lr.fit(X_train, y_train)
    sk_y_pred = sk_lr.predict_proba(X_test)[:, 1]
    
    mean_y_pred_Xh = np.median(Xh_test, axis=1)

    mean_y_pred = np.mean(X_test, axis=1)

    fmax_score(y_test, y_pred, display=True)
    fmax_score(y_test, mean_y_pred_Xh, display=True)
